...
public class QLearningController extends Controller {
	...
	static final double GAMMA_DISCOUNT_FACTOR = 0.9; /* Must be < 1, small values make it very greedy */
	static final double LEARNING_RATE_CONSTANT = 0.6; /* See alpha(), lower values are good for quick results in large and deterministic state spaces */
		...
		if (!paused) {
			...
			if (previous_state != null) {
				...
				if (Qtable.get(prev_stateaction) == null) {
					...
					double oldQValue = Qtable.get(prev_stateaction);
					Qtable.put(prev_stateaction,oldQValue + LEARNING_RATE_CONSTANT*(previous_reward + (GAMMA_DISCOUNT_FACTOR*getMaxActionQValue(new_state))-oldQValue));
				}
				
				/* TODO: IMPLEMENT Q-UPDATE HERE! */
				
				
				/* See top for constants and below for helper fudwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwnctions */
